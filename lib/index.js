...
function clientsuccess(username, password) {
	let methods = {
		...
		clients: {
			...
			customfield: (clientId, params) => api.patch(`/clients/${clientId}`, params)
		},
		contacts: {
		...
		clientStatus: (params) => api.get(`/client-statuses`, params),
		subscriptions: {
			list: (clientId, params) => api.get(`/subscriptions?clientId=${clientId}`, params),
			create: (params) => api.post('/subscriptions', params),
			update: (subscriptionId, params) => api.put(`/subscriptions/${subscriptionId}`, params),
			renew: (subscriptionId, params) => api.put(`/subscriptions/${subscriptionId}`, params),
			delete: (subscriptionId, params) => api.delete(`/subscriptions/${subscriptionId}`, params)
		}
	};
	...
	function handleRefresh(originalResponse) {
		return new Promise((resolve, reject) => {
			if (originalResponse.status === 401 || (originalResponse.data.error && originalResponse.data.error.includes('invalidToken'))) {
				rejectedCalls.push({
					config: originalResponse.config,
					resolve,
					reject
				});
